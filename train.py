...
parser.add_argument('--dataset_name', default='en-ontonotes', type=str)
...
demo = False
if demo:
    cache_fn = f"/kaggle/working/caches/data_{bart_name}_{dataset_name}_{target_type}_demo.pt"
else:
    cache_fn = f"/kaggle/working/caches/data_{bart_name}_{dataset_name}_{target_type}.pt"

@cache_results(cache_fn, _refresh=False)
def get_data():
    pipe = BartNERPipe(tokenizer=bart_name, dataset_name=dataset_name, target_type=target_type)
    if dataset_name == 'conll2003':
        paths = {'test': "data/notov4/test.txt",
                 'train': "data/notov4/train.txt",
                 'dev': "data/notov4/dev.txt"}
        data_bundle = pipe.process_from_file(paths, demo=demo)
    elif dataset_name == 'en-ontonotes':
        paths = {'test': "data/en-ontonotes/test.txt",
                 'train': "data/en-ontonotes/train.txt",
                 'dev': "data/en-ontonotes/dev.txt"}
        # paths = 'data/notov4'
...
tester = Tester(data=test_dataset, model=model, batch_size=batch_size*2, metrics=metric, metric_key='f')
